#!/usr/bin/env python3
"""
Aplicación de Realidad Aumentada con OpenCV y Streamlit
Archivo único sin dependencias externas de Streamlit
"""
    ...
    if canvas_result is not None and canvas_result.json_data is not None:
        for obj in canvas_result.json_data.get("objects", []):
            if obj.get("type") == "rect":
                ...
                l = int(round(obj.get("left", 0)))
                t = int(round(obj.get("top", 0)))
                if obj.get("originX") == "center":
                    l -= w // 2
                if obj.get("originY") == "center":
                    t -= h // 2
                l = max(0, min(l, pil_img.width - 1))
                t = max(0, min(t, pil_img.height - 1))
                w = max(1, min(w, pil_img.width - l))
                h = max(1, min(h, pil_img.height - t))
                rect = (l, t, l + w, t + h)
                tracker.set_rect(rect)
                st.success(f"ROI: {rect}")

st.image(processed_frame_rgb, channels="RGB", caption="Video")